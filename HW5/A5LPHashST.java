 ...
 *  @author Kevin Wayne
 */
public class A5LPHashST<Key, Value> implements Iterable<Key> {
     ...
     */
    public A5LPHashST(int capacity) {
        m = capacity;
        n = 0;
    ...
    private void resize(int capacity) {
        A5LPHashST<Key, Value> temp = new A5LPHashST<Key, Value>(capacity);
        for (int i = 0; i < m; i++) {
            if (keys[i] != null) {
    ...
    public static void main(String[] args) { 
        A5LPHashST<String, Integer> st = new A5LPHashST<String, Integer>();
    ...
    class Iterator implements java.util.Iterator<Key> {
		...
		public Key next() {
			if (hasNext())
				return A5LPHashST.this.keys[current++];
			else
